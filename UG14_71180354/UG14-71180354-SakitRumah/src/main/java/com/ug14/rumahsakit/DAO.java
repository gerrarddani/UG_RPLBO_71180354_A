...
public class DAO {
    ...
    public void inputJadwal(Jadwal jadwal) {
        String sql = "INSERT INTO jadwal values " + "('" + jadwal.getPasien().getRm() + "', " + jadwal.getDokter().getIdDokter() + ", " + jadwal.getSuster().getIdSuster() + ", " + jadwal.getPelayanan().getIdPelayanan() + ", 0);";
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:rumahsakit.db");
             Statement stmt = conn.createStatement()) {
            stmt.execute(sql);
        } catch (SQLException e) {
